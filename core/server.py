                    ...
                    if cmd == "kanban":
                        ...
                        await websocket.send(
                            json.dumps(
                                {
                                    "type": "models",
                                    "data": {"provider": cur, "models": models, "current": os.environ.get("MODEL_NAME", cfg.get("default_model", ""))},
                                }
                            )
                        )
                        ...
                        await websocket.send(
                            json.dumps({"type": "experts", "data": {"available": _available_providers(), "profiles": self.agent.orchestra.profiles}})
                        )
                        ...
                        await websocket.send(
                            json.dumps({"type": "changeset_update", "data": {"id": cs_id, "note": note, **self.agent.changesets.summary(cs_id)}})
                        )
                        ...
                        if p and p.is_file():
                            try:
                                await websocket.send(
                                    json.dumps({"type": "file_content", "data": {"path": rel, "content": p.read_text(errors="replace")[:1_000_000]}})
                                )
                            except OSError as exc:
