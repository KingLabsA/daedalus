...
def default_embed_fn(texts: List[str]) -> Optional[List[List[float]]]:
    ...
    try:
        for text in texts:
            r = requests.post(f"{host}/api/embeddings",
                              json={"model": model, "prompt": text[:4000]}, timeout=30)
...
class EmbeddingIndex:
    def __init__(self, root: str = ".", embed_fn: Callable = default_embed_fn,
                 cache_path: str = ".hermes/embed_cache.json"):
        self.root = Path(root)
    ...
    def search(self, query: str, k: int = 8) -> List[Dict]:
        ...
        return [
            {"file": c["file"], "line": c["line"], "score": round(s, 4), "preview": c["text"]}
