...
class Vision:
    def __init__(self, vision_chat_fn: Callable[[list], str], frame_extractor: Optional[Callable] = None):
            ...
            with tempfile.TemporaryDirectory(prefix="hermes_video_") as tmp:
                ...
                for i, frame in enumerate(frames, 1):
                    desc = str(
                        self.vision_chat_fn(
                            self._image_message(
                                f"Frame {i}/{len(frames)} of a video. Describe what is happening.", [frame]
                            )
                        )
                    )
                    descriptions.append(f"Frame {i}: {desc}")
                summary_prompt = (
                    f"These are sequential frame descriptions from one video. Answer: {question}\n\n"
                    + "\n".join(descriptions)
                )
