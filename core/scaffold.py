        ...
        "package.json": json.dumps(
            {
                ...
                "devDependencies": {"@sveltejs/kit": "^2.5.0", "@sveltejs/adapter-auto": "^3.2.0", "svelte": "^4.2.0", "vite": "^5.4.0"},
            },
            indent=2,
        ),
...
def official(kind: str, name: str, which: Callable[[str], object] = None) -> dict:
    ...
    if cmd:
        ...
        if which(launcher):
            return {"ok": True, "mode": "official", "kind": kind.lower(), "command": cmd, "dir": _slug(name), "note": f"Run the maintained generator: {cmd}"}
