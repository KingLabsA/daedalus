...
def test_registry_execute_parallel():
    ...
    calls = [{"id": "1", "name": "identity", "args": {"x": "a"}}, {"id": "2", "name": "identity", "args": {"x": "b"}}]
...
def test_plugin_discover_local(tmp_path):
    ...
    (tmp_path / "myplugin" / "plugin.json").write_text(
        json.dumps({"name": "myplugin", "version": "1.0.0", "description": "desc", "author": "me", "tools": ["x"], "min_agent_version": "1.0.0"})
    )
